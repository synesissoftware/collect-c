/* /////////////////////////////////////////////////////////////////////////
 * File:    collect-c/terse/dlist.h
 *
 * Purpose: Doubly-linked list container terse api.
 *
 * Created: 7th February 2025
 * Updated: 16th February 2025
 *
 * ////////////////////////////////////////////////////////////////////// */


/* /////////////////////////////////////////////////////////////////////////
 * includes
 */

#include <collect-c/dlist.h>


/* /////////////////////////////////////////////////////////////////////////
 * terse-form macros
 */

#define CLC_DL_F_NO_SPARES                                  COLLECT_C_DLIST_F_NO_SPARES

#define CLC_DL_define_empty                                 COLLECT_C_DLIST_define_empty

#define CLC_DL_free                                         COLLECT_C_DLIST_free_storage

#define CLC_DL_is_empty                                     COLLECT_C_DLIST_is_empty
#define CLC_DL_len                                          COLLECT_C_DLIST_len
#define CLC_DL_spare                                        COLLECT_C_DLIST_spare

#define CLC_DL_cfront_t                                     COLLECT_C_DLIST_cfront_t

#define CLC_DL_push_back_by_ref                             COLLECT_DLIST_push_back_by_ref
#define CLC_DL_push_back_by_val                             COLLECT_DLIST_push_back_by_val
#define CLC_DL_push_front_by_ref                            COLLECT_DLIST_push_front_by_ref
#define CLC_DL_push_front_by_val                            COLLECT_DLIST_push_front_by_val

#define CLC_DL_clear                                        COLLECT_C_DLIST_clear
#define CLC_DL_erase_node                                   COLLECT_C_DLIST_erase_node
#define CLC_DL_insert_after                                 COLLECT_C_DLIST_insert_after
#define CLC_DL_insert_before                                COLLECT_C_DLIST_insert_before

#define CLC_DL_front_t                                      COLLECT_C_DLIST_front_t
#define CLC_DL_back_t                                       COLLECT_C_DLIST_back_t
#define CLC_DL_cfront_t                                     COLLECT_C_DLIST_cfront_t
#define CLC_DL_cback_t                                      COLLECT_C_DLIST_cback_t


/* /////////////////////////////////////////////////////////////////////////
 * inclusion control
 */

#pragma once


/* ///////////////////////////// end of file //////////////////////////// */

